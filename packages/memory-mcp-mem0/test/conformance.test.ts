...
// Mirrors the sqlite conformance test: drive all 6 §8.1 tools through
// an in-process server and ajv-validate every response shape. Target:
// 100% pass rate across canonical + error branches.
...
describe("ajv conformance — §8.1 + L-58 response shapes", () => {
  it("all 6 tool responses + sensitive-personal branch validate (100% pass rate)", async () => {
    const backend = new Mem0Backend({ client: createFakeMem0Client() });
    const app = await buildMem0Server({ backend });
    try {
      const added = (await backend.addMemoryNote({
        summary: "conformance seed",
        data_class: "internal",
        session_id: "ses_mem0_conf000001",
        note_id: "mem_conformance"
      ...
      const nf = await backend.readMemoryNote({ id: "mem_does_not_exist_99" });
      ...
      const forbidden = (await backend.addMemoryNote({
        ...
        session_id: "ses_mem0_conf_pii0001"
